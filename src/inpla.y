...
#define BUILT_DATE  "26 March 2026"
...
HoopList *HoopList_new_forName(unsigned int size) {
  ...
  if (size > MAX_HOOP_SIZE) {
    ...
    fprintf(stderr, "Requested size for Name Hoop (%u) exceeds the maximum limit MAX_HOOP_SIZE (%u).\n", size, MAX_HOOP_SIZE);
...
HoopList *HoopList_new_forAgent(unsigned int size) {
  ...
  if (size > MAX_HOOP_SIZE) {
    fprintf(stderr, "\n[Inpla VM Fatal Error] Memory limit exceeded!\n");
    fprintf(stderr, "Requested size for Agent Hoop (%u) exceeds the maximum limit MAX_HOOP_SIZE (%u).\n", size, MAX_HOOP_SIZE);
    fprintf(stderr, "Possible infinite loop detected in Interaction Rules!\n");
    exit(-1); // Exit safely with an error code    
  }
...
loop_a2IsFixnum:    
	...
	//
	// Note: Successful matches inside this file will jump back
	// via `goto loop;` or `return;`.
	// If no match is found, execution falls through
	// to the runtime error below.	
	#include "builtin_OP-INT.inc.c"
	// ------------------------------------------------
#endif
...
loop_agent_a1_a2_this_order:
...
#ifdef INPLA_USE_BUILTINS
	// ------------------------------------------------
	// built-in: a1><a2 is BUILT-IN_OP >< AGENT
	// where ID(a1) >= ID(a2).
	// It has a lot of matches.
	//
	// Note: Successful matches inside this file will jump back
	// via `goto loop;` or `return;`.
	// If no match is found, execution falls through
	// to the next operation of wildcard agents.
